...
from mykg.pass2_concat import concat_file_order
...
def run_pass2_concat(
    ...
    """Concat variant of run_pass2_batched.

    Identical real-source_file batching machinery and shard format as
    run_pass2_batched — the ONLY difference is chunk ORDER: files are ordered by
    directory + filename-prefix (via concat_file_order) before chunking, so that
    related small files land in the same token-bounded batch and the LLM sees them
    together. There are NO virtual batches: every chunk carries its real
    source_file, and shards are keyed by real filenames exactly like
    run_pass2_batched. run_pass2_batched is intentionally left untouched.

    Returns (raw_extractions, chunk_node_index, failed_chunks, batch_map).
    """
    if max_workers is None:
        max_workers = _cfg.PASS2_MAX_WORKERS
    gate = error_gate if error_gate is not None else noop_gate()
    ...
    # Chunk all files upfront to build the batch plan. The ONLY difference from
    # run_pass2_batched: order files by directory + filename-prefix so related
    # files batch together (concat's cross-file-context behaviour).
    all_chunks: list[Chunk] = []
    for fname in concat_file_order(files):
        all_chunks.extend(chunk_file(fname, files[fname]))
    ...
    total_files = len(files)
    log.info(
        "Pass 2 concat — %d file(s), %d chunk(s) → %d batch(es)",
        total_files,
        len(all_chunks),
        total_batches,
    )
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                gate.record_error(exc)
                log.error("Pass 2 concat — batch %d failed: %s", batch_idx_from_future, exc)
                completed.append((batch_idx_from_future, None, batches[batch_idx_from_future]))
                _flush_progress(batch_name, None, str(exc))
            ...
            eta_m = int((remaining_secs % 3600) // 60)
            log.info(
                "Pass 2 concat progress — %d/%d batches (%.1f%%) — elapsed %s — ETA ~%dh %02dm",
                done_batches,
                total_batches,
                done_batches / total_batches * 100 if total_batches else 0.0,
                _fmt_elapsed(elapsed),
        ...
        for retry_round in range(batch_retry_max):
            ...
            if not failed_items:
                break
            log.info(
                "Pass 2 concat — retry round %d/%d: %d failed batch(es): %s",
                retry_round + 1,
                batch_retry_max,
                len(failed_items),
                [bi for bi, _ in failed_items],
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as retry_executor:
                ...
                for future in as_completed(retry_futures):
                    ...
                    try:
                        ...
                        _flush_progress(bname, result[1], None)
                        log.info(
                            "Pass 2 concat — retry round %d batch %d succeeded",
                            retry_round + 1,
                            bi,
                        )
                    except Exception as exc:
                        gate.record_error(exc)
                        log.error(
                            "Pass 2 concat — retry round %d batch %d still failed: %s",
                            retry_round + 1,
                            bi,
                            exc,
                        )
        ...
        for batch_idx, extraction, batch in completed:
            ...
            # All nodes/edges from a mixed batch are attributed to each constituent
            # file; assembler dedup collapses the duplication (same as run_pass2_batched).
            for chunk in batch:
                fname = chunk.source_file
                chunk_idx_1based = chunk.chunk_index + 1
                chunk_node_index[fname][str(chunk_idx_1based)] = [
