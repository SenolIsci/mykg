...
- **Human-in-the-loop ontology design** — run with `--review` to pause after schema induction; inspect and edit `schema.json`, then resume with `mykg approve-schema`. For a Protégé-based workflow, open `intermediate/schema.ttl`, modify the ontology, save, and re-run with `--base-schema intermediate/schema.ttl --freeze-schema` to extract against your revised ontology as-is
